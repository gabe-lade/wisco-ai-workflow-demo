# ============================================================================
# Script: ohio_tax_analysis.R
# Purpose: Analyze Ohio school district property tax data and create
#          visualizations of 20-mill floor impacts
# Author: Gabriel Lade
# Date: 2024-12-01
# ============================================================================
...
ggsave("output/figures/ohio_school_districts_floor_map.png",
...
ggsave("output/figures/greene_county_tax_comparison.png",
...
# Count floor districts per county and take the top 10
top_counties <- sd_data %>%
  filter(at_floor == TRUE) %>%
  count(county_name, name = "n_floor_districts") %>%
  slice_max(n_floor_districts, n = 10, with_ties = FALSE) %>%
  # Order counties by count for plotting
  mutate(county_name = fct_reorder(county_name, n_floor_districts))

p3 <- ggplot(top_counties, aes(x = county_name, y = n_floor_districts)) +
  geom_col(fill = "#ba0c2f") +
  geom_text(aes(label = n_floor_districts), hjust = -0.3, size = 3.5) +
  coord_flip() +
  scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
  labs(
    title = "Top 10 Ohio Counties by Districts at 20-Mill Floor",
    subtitle = "Number of school districts at the minimum property tax rate",
    x = NULL,
    y = "Number of Districts at Floor",
    caption = "Source: Ohio Department of Education, Tax Year 2024"
  ) +
  theme_minimal(base_size = 12) +
  theme(
...
ggsave("output/figures/top_counties_floor_districts.png",
       p3, width = 10, height = 6, dpi = 300, bg = "white")

cat("\n✓ Top counties bar chart saved\n")
cat("\nAll outputs saved to output/figures/\n")
